...
| The update feed is this repository | It is `epos-releases`, a separate public repository holding releases and no code — so the source can go private without a token ending up inside every till, where anybody could extract it and read the source with it |
