...
81 tests, no database and no network. Its handlers take a parsed body and a store
...
| Chinese on a ticket | That a note in Chinese goes out as a **bitmap and not as code-page bytes** — the defect a real kitchen ticket showed, where the dish name printed and the note beneath it was rubbish; that an ASCII line is still byte-for-byte plain text, because a ticket whose layout shifted would be worse than the bug being fixed; that a shop which turned rasterising off still gets text; and that the raster fallback does not call back into the line that called it, which would empty the stack mid-service |
...
| What the till records | That an amendment saying exactly what the last entry said is **not written**, while one that changes something still is, and a void is recorded even when the figures did not move; that an order writes `placed`, then `amended`, then `paid` — the verb **derived** from what changed rather than declared by a caller, because a log callers must remember to write is a log with holes; that a draft nobody has committed to writes nothing, since a ticket is saved on nearly every keystroke; that every tender gets its own entry exactly once, because a split payment recorded only as a total cannot be reconciled against a card terminal's report; that a void and a refund are different events; that a shift records both the counted and the expected cash; and that an order and its money land in one unbroken chain |
| The chain, across two languages | That `cloud/src/chain.ts` recomputes exactly what `ChangeChain` does — pinned to constants **printed by the C# implementation**, not worked out in TypeScript. The first attempt produced `Z` where .NET produces `+00:00`, and three fractional digits where .NET writes seven; both spell the same instant and only one hashes to what the till wrote |
| The log going up | That entries ride on the same call as the entitlement; that the watermark follows what the cloud says it stored rather than what was sent; that a batch sent twice is stored once; that a **gap is refused, recorded and does not move the watermark** — while the entitlement still comes back, because a broken chain is ours to look at and not a reason to stop a shop trading; and that with nothing pending and nothing due, no request is made at all |
| The bundle from the cloud | That only the **version** rides on a sync and the menu itself comes from its own call, so a shop whose menu has not changed downloads nothing; that the version is the hash, so uploading the same file twice does not look like a change; that a file which is not a shop bundle is refused **before** it is stored, since a broken one would reach every till and each would refuse it with nothing on the service saying why; that minting a new activation code leaves the menu version alone; and that a downloaded bundle is left on disk for the next start rather than applied while somebody could be mid-sale |
| Updates | That with no feed configured — the shipped state — nothing is checked, nothing applied and nothing logged; that an installation Velopack does not manage (a developer running `dotnet run`) is left alone rather than told about updates it could never apply; and that a malformed feed cannot stop a till opening |
